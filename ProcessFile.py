...
def prediction_linear(reg2, reg3):
    ...
    tmp_total = 0
    print('\n', file=log)
    print('Linear Prediction', file=log)
    for j in range(0, len(test1)):
        ...
        print ('Predicted Total Area: ', predict_area, ' Predicted Unit Price: ', predict_price, ' Monthly Revenue: ', monthly_revenue,file = log)
    print ('Whole Year Total Revenue: ', tmp_total, file=log)
    ...
    fig.suptitle(data.sheet_names()[sh], fontweight = 'bold')
    # plt.subplot(4,1,1)
    ...
    plt.savefig(savePath+data.sheet_names()[sh]+'_linear.png')
    # plt.show()
...
def one_vs_one_poly():
    ...
    poly2 = Pipeline([('poly', PolynomialFeatures(degree=3)), ('linear', LinearRegression(fit_intercept=False))])
...
def prediction_poly(poly1, poly2):
    ...
    print('\n', file=log)
    print('Poly Prediction', file=log)
    for j in range(0, len(test1)):
    ...
    plt.plot(total_revenue)
    plt.plot(plot_all_predict_monthly_rev, 'r')
    plt.savefig(savePath + data.sheet_names()[sh] + '_poly.png')
    # plt.show()
...
for sh in range(0, data.nsheets):
    ...
    get_data_array(data.sheet_by_index(sh))
    ...
    print ("City Name: ", data.sheet_names()[sh], file=log)
